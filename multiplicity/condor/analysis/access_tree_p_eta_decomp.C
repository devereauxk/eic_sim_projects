...
void access_tree_p_eta_decomp()
{
  ...
  for (int i = 0; i < num_species; i++) {
    ...
    //load new out file
    outFile = dirs[i] + "p_eta_decomp.root";
    fout = new TFile(outFile.c_str(), "recreate");

    //Initialize new histogram
    kaon_pos = new TH2D("kaon_pos","particle p vs pseudo-rapidity, kaon pos",100,0,20,80,-10,10);
    kaon_pos->Sumw2();
    kaon_neg = new TH2D("kaon_neg","particle p vs pseudo-rapidity, kaon neg",100,0,20,80,-10,10);
    kaon_neg->Sumw2();
    pion_pos = new TH2D("pion_pos","particle p vs pseudo-rapidity, pion pos",100,0,20,80,-10,10);
    pion_pos->Sumw2();
    pion_neg = new TH2D("pion_neg","particle p vs pseudo-rapidity, pion neg",100,0,20,80,-10,10);
    pion_neg->Sumw2();
    proton_pos = new TH2D("proton_pos","particle p vs pseudo-rapidity, proton pos",100,0,20,80,-10,10);
    proton_pos->Sumw2();
    proton_neg = new TH2D("proton_neg","particle p vs pseudo-rapidity, proton neg",100,0,20,80,-10,10);
    proton_neg->Sumw2();
        ...
        if (status == 1) {
          switch(id) {
            ...
            case -211:
              ...
              break;
            case 2212:
              proton_pos->Fill(p, eta);
              break;
            case -2212:
              proton_neg->Fill(p, eta);
              break;
            default: break;
          }
        }
