...
TH2D* h2d_p_vs_eta = NULL;
TH1D* h1d_p_in_eta[etabin] = {0};
TGraphErrors* g_p_vs_eta = NULL;
...
std::string inFileName = "p_eta_total_TH2D.root";
...
void slice_2D_hist()
{
  for (int ieta = 0; ieta < etabin; ++ieta)
  {
    ...
    h2d_p_vs_eta->GetYaxis()->SetRangeUser(eta_lo[ieta], eta_hi[ieta]);
    ...
    h1d_p_in_eta[ieta] = (TH1D*)h2d_p_vs_eta->ProjectionX( Form("h1d_p_in_eta_%d",ieta) );
    h1d_p_in_eta[ieta]->SetName( Form("h1d_p_in_eta_%d",ieta) );
    h1d_p_in_eta[ieta]->SetTitle( "particle p ditributiton" );
  }
  // set back to original range
  h2d_p_vs_eta->GetYaxis()->SetRangeUser(-3,3);
}

void plot_histograms_p_eta_totals()
{
  TFile * fin;
  std::string inFile;
  for (int i = 0; i < num_species; i++) {
    ...
    fin->ls();
    h2d_p_vs_eta = (TH2D*)fin->Get("h2d_p_vs_eta");
    slice_2D_hist();

    // plots p and eta 2d histogram
    TCanvas* c3 = new TCanvas("c3","c3",800,800); // create new canvas
    c3->Range(0,0,1,1);
    c3->SetLeftMargin(0.15);
    c3->SetRightMargin(0.15);
    c3->SetBottomMargin(0.1);
    h2d_p_vs_eta = (TH2D*) h2d_p_vs_eta->Rebin2D(2, 2);
    h2d_p_vs_eta->SetTitle(names[i].c_str());
    h2d_p_vs_eta->GetXaxis()->SetTitle("<p> [GeV/c]");
    h2d_p_vs_eta->GetYaxis()->SetTitle("#eta");
    h2d_p_vs_eta->GetXaxis()->SetTitleOffset(1.3);
    h2d_p_vs_eta->GetYaxis()->SetTitleOffset(1.5);
    c3->SetLogz();
    h2d_p_vs_eta->Draw("colz");
    ...
    for (int ieta = 0; ieta < etabin; ++ieta)
    {
      h1d_p_in_eta[ieta]->SetStats(0); // not showing the box on the top right corner
      h1d_p_in_eta[ieta]->SetLineColor(eta_color[ieta]); // eta_color array defined at the beginning
      h1d_p_in_eta[ieta]->SetMarkerColor(eta_color[ieta]);
      h1d_p_in_eta[ieta]->Draw("hsame");
      leg->AddEntry(h1d_p_in_eta[ieta],Form("%.0f < #eta < %.0f",eta_lo[ieta],eta_hi[ieta]),"l"); // "l" means line
    }
    leg->Draw("same");
    ...
    c3->SaveAs( Form("%sp_eta_binned_single.pdf", outdirs[i].c_str()) );
    ...
    for (int ieta = 0; ieta < etabin; ++ieta)
    {
      c_all->cd(ieta+1); h1d_p_in_eta[ieta]->Draw("hsame");
      TLatex* tl = new TLatex();
      tl->SetTextAlign(11);
      tl->SetTextSize(0.035);
      tl->SetTextColor(kBlack);
      ...
      h1d_p_in_eta[ieta]->GetXaxis()->SetTitle("<p> [GeV/c]");
      h1d_p_in_eta[ieta]->GetYaxis()->SetTitle("counts");
      h1d_p_in_eta[ieta]->GetXaxis()->SetTitleOffset(1.3);
      h1d_p_in_eta[ieta]->GetYaxis()->SetTitleOffset(1.5);
      h1d_p_in_eta[ieta]->GetXaxis()->SetRangeUser(0,10);
      h1d_p_in_eta[ieta]->SetTitle("");
      h1d_p_in_eta[ieta]->SetStats(0); // not showing the box on the top right corner
      h1d_p_in_eta[ieta]->SetLineColor(eta_color[ieta]); // eta_color array defined at the beginning
      h1d_p_in_eta[ieta]->SetMarkerColor(eta_color[ieta]);
    }
    c_all->SaveAs(Form("%sp_eta_binned_decomp.pdf", outdirs[i].c_str()));

    // plot average p as function of eta using TGraph
    double temp_p[etabin] = {0}, temp_p_err[etabin] = {0};
    double temp_eta[etabin] = {0}, temp_eta_err[etabin] = {0};
    for (int ieta = 0; ieta < etabin; ++ieta)
    {
      temp_p[ieta] = h1d_p_in_eta[ieta]->GetMean();
      temp_eta[ieta] = 0.5*(eta_lo[ieta]+eta_hi[ieta]);
    }
    g_p_vs_eta = new TGraphErrors(etabin,temp_eta,temp_p,temp_eta_err,temp_p_err);
    g_p_vs_eta->SetMarkerColor(kRed);
    g_p_vs_eta->SetMarkerStyle(20);
    g_p_vs_eta->SetMarkerSize(1);
