...
std::string inFileName = "p_eta_decomp.root";
...
void slice_2D_hist()
{
  for (int ieta = 0; ieta < etabin; ++ieta)
  {
    ...
    h1d_pt_in_eta[ieta] = (TH1D*)h2d_pt_vs_eta->ProjectionX( Form("h1d_p_in_eta_%d",ieta) );
    h1d_pt_in_eta[ieta]->SetName( Form("h1d_p_in_eta_%d",ieta) );
    h1d_pt_in_eta[ieta]->SetTitle( "particle p ditributiton" );
  }
...
void plot_histograms_p_eta_decomp()
{
  for(int i_titles; i_titles < num_histograms; i_titles++) {
    ...
    for (int i = 0; i < num_species; i++) {
      ...
      h2d_pt_vs_eta->GetXaxis()->SetTitle("<p> [GeV/c]");
      ...
      c3->SaveAs(Form("%sp_vs_eta_2D_histo_%s.pdf", outdirs[i].c_str(), histogram_titles[i_titles].c_str()));
      ...
      c3->SaveAs( Form("%sp_eta_binned_single_%s.pdf", outdirs[i].c_str(), histogram_titles[i_titles].c_str()) );
