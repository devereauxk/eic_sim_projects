    ...
    void SetNorm(const int option = 0)
    {
      for (int iQ2 = 0; iQ2 < Q2bin; ++iQ2)
      {
        for (int ivar = 0; ivar < varbin; ++ivar)
        {
          ...
          //cout << "ep normalization factor: " << norm_ep << " ; eA normalization factor: " << norm_eA << endl;
...
void plot_chadron_gen(const char* inFile_ep = "hists_gen_ep.root", const int sys_ep_option = 0, const int energy_ep_option = 0, const char* inFile_eA = "hists_gen_eA.root", const int sys_eA_option = 0, const int energy_eA_option = 0, const char* outFile = "hists_gen.root", const int charm_norm_option = 1)
{
  mcs(-1);
  ...
  plot_D0.PlotEvents();
  plot_D0.SetNorm(charm_norm_option);
  // plot_D0.Plot2D();
  plot_D0.SliceInEta();
  plot_D0.Plot1D();
  plot_D0.PlotComparison();

  PlotHadron plot_Lc(4122,sys_ep_option,energy_ep_option,sys_eA_option,energy_eA_option,var_option);
  plot_Lc.ReadEvtHists(fin_ep,fin_eA);
  plot_Lc.ReadHadronHists(fin_ep,fin_eA);
  plot_Lc.PlotEvents();
  plot_Lc.SetNorm(charm_norm_option);
  // plot_Lc.Plot2D();
  plot_Lc.SliceInEta();
  plot_Lc.Plot1D();
