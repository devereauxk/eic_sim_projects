...
{ // smear_2nd_vtx & momentum: 0--no smearing, 1--DM smearing, 2--LBL smearing, 3--Hybrid smearing, 4--ATHENA smearing
  ...
  // ATHENA smeaing
  TFile* f_athena_tracks = new TFile("ATHENA_Resolutions.root","READ");
  ...
  for(Int_t ievt = 0; ievt < nevt; ievt++)
  {
    ...
    if (smear_option==4)
    {
      ...
      for (int ipart = 0; ipart < nParticles; ++ipart)
      {
        ...
        if (abs(part->Id())!=2212 && abs(part->Id())!=211 && abs(part->Id())!=321) continue; // proton, pion, kaon
        if(fabs(part->GetEta())<3.5 && part->GetP()>0.5) multi_charged_hadron_35++;
        if(fabs(part->GetEta())<1.0 && part->GetP()>0.5) multi_charged_hadron_10++;
      }
      evt_vtx_reco = smearPVTATHENA(evt_vtx, multi_charged_hadron_35); // smear transverse direction
      evt_vtx_reco = smearPVZATHENA(evt_vtx, multi_charged_hadron_10); // smear longitudinal direction
    }
