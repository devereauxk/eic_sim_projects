  ...
  for (int ieta = 0; ieta < etabin; ieta++)
  {
    for (int ipt = 0; ipt < ptbin; ipt++)
    {
      mclogxy(cno++);
      {
        ...
        tl->DrawLatexNDC(0.22,0.84,"eHIJING, e+Au @ 10+100 GeV, 2 #cdot 10^{8} events");
...
void pt_bin_side_by_side()
{
  ...
  mclogx(cno++);
  {
    ...
    tl->SetTextSize(0.028);
    tl->SetTextColor(kBlack);
    tl->DrawLatexNDC(0.22,0.84,Form("eHIJING, %s @ 10+100 GeV, 2 #cdot 10^{8} events", species[species_pick]));
...
void nuclei_hists()
{
  ...
  mclogx(cno++);
  {
    ...
    tl->SetTextSize(0.028);
    tl->SetTextColor(kBlack);
    tl->DrawLatexNDC(0.22,0.84,"eHIJING, e+A @ 10+100 GeV, 2 #cdot 10^{8} events");
    tl->DrawLatexNDC(0.22,0.81,Form("#eta #in [%.1f, %0.1f)",eta_lo[etabin_pick],eta_hi[etabin_pick]));
...
void energy_hists()
{
  ...
  mclogx(cno++);
  {
    ...
    tl->SetTextSize(0.028);
    tl->SetTextColor(kBlack);
    tl->DrawLatexNDC(0.22,0.84,Form("eHIJING, e+Au, 2 #cdot 10^{8} events, K=%i",k[k_pick]));
    tl->DrawLatexNDC(0.22,0.81,Form("#eta #in [%.1f, %0.1f)",eta_lo[etabin_pick],eta_hi[etabin_pick]));
    tl->DrawLatexNDC(0.22,0.78,Form("p_{T,jet} #in [%.1f, %0.1f)",pt_lo[ptbin_pick],pt_hi[ptbin_pick]));
...
void power_hists()
{
  ...
  mclogx(cno++);
  {
    ...
    tl->SetTextSize(0.028);
    tl->SetTextColor(kBlack);
    tl->DrawLatexNDC(0.22,0.84,"eHIJING, e+Au, 2 #cdot 10^{8} events");
    tl->DrawLatexNDC(0.22,0.81,Form("#eta #in [%.1f, %0.1f)",eta_lo[etabin_pick],eta_hi[etabin_pick]));
    tl->DrawLatexNDC(0.22,0.78,Form("p_{T,jet} #in [%.1f, %0.1f)",pt_lo[ptbin_pick],pt_hi[ptbin_pick]));
