...
static char* fname_eAu_by_K[knum] = {(char*)"./eHIJING/eAu_1E8_K0_condor_v2/merged.root", (char*)"./eHIJING/eAu_1E8_K2_pow05/merged.root", (char*)"./eHIJING/eAu_1E8_K4_pow05/merged.root", (char*)"./eHIJING/eAu_1E8_condor_pow05/merged.root"};
...
void pt_bin_side_by_side()
{
  ...
  mclogx(cno++);
  {
    ...
    for (int ipt = 0; ipt < ptbin-2; ipt++)
    {
      ...
      temp->Print();
      cout<<" bin 27 content: "<<temp->GetBinContent(27)<<cout;
      fig3.push_back(temp);
      temp_baseline->Print();
      cout<<" baseline bin 27 content: "<<temp_baseline->GetBinContent(27)<<cout;
...
void pt_spectra()
{
  ...
  for (int ipt = 0; ipt < ptbin-2; ipt++)
  {
    ...
    int bin_hi = temp->FindBin(pt_hi[ipt]);
    double integral = temp->Integral(bin_lo, bin_hi);
    double sum = 0;
    for (int ibin = bin_lo; ibin < bin_hi; ibin++)
    {
      sum += temp->GetBinCenter(ibin) * temp->GetBinContent(ibin);
    }
    double avg = sum / integral;
