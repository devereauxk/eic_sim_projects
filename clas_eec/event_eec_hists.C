...
const int ptbin = 4; // inclusive on last bin, inclusive on lower limit, exclusive on upper
static double pt_lo[ptbin] = {0.2, 2, 5, 0};
static double pt_hi[ptbin] = {2, 5, 10, 15};

const int etabin = 4; // inclusive on last bin, inclusive on lower limit, exclusive on upper
static double eta_lo[etabin] = {0, 1, 2, 0};
static double eta_hi[etabin] = {1, 2, 3, 3};
    ...
    double eec_weight_power = 1, int calc_Q2x = 0)
{
  ...
  while (iline < nlines)
  {
    ...
    while (iline < nlines && stoi(content[iline][0]) == ievt)
    {
      ...
      if (part.Theta() > 10 && part.Theta() < 70 && Id!=11)
      {
    ...
    // assuming 10-70 degree coverage
    double eta_high = 2.4362;
    double eta_lo = 0.35637;

    // jet processing
    for (unsigned ijet = 0; ijet < jets.size(); ijet++)
    {
      // cuts on jet kinematics
      if (jets[ijet].pt() < 0.2 || abs(jets[ijet].eta()) > eta_high || abs(jets[ijet].eta()) < eta_lo) continue;
      ...
      {
        if (constituents[iconstit].pt() < 0.2) continue;
