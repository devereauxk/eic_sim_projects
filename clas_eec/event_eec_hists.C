    ...
    double eec_weight_power = 1)
{
  ...
  while (iline < nlines)
  {
    ...
    while (iline < nlines && stoi(content[iline][0]) == ievt)
    {
      ...
      cout<<"PART (theta, id, E, charge) "<<part.Theta()<<" "<<Id<<" "<<part.E<<" "<<Charge<<endl;
...
void event_eec_hists(const char* inFile = "merged.root", const char* outFile = "hists_eec.root", const int gen_type = 1,
