...
C...Local arrays.
     ...
     &CEFFD(10,9),SIGTMP(6,0:5),EXPO(4)
...
C...Characteristic class masses, slope parameters, beta = sqrt(X).
      ...
      DATA EXPO/2.575D0,2.575D0,2.4D0,2.55D0/
...
C...change of elastic slope parameter for rho, phi and J/psi (08/2010)
C        SIGTMP(I,0)=XPAR(IPR)*SEPS+YPAR(IPR)*SETA
C If we include the q2 dependence of the deltafor rho, we need also 
C to change xpar
C          seps=s**(0.173+0.068*dlog(VINT(307)/(PMVIRT**2)+1.D0))
        IF(IHA.eq.2) then 
          PMVIRT=PMAS(PYCOMP(113),1)
          SIGTMP(I,0)=XPAR(IPR)*SEPS+YPAR(IPR)*SETA
        BEL=5.84/(1+(PARP(165))*(VINT(307)/(PMVIRT**2))**PARP(166))+4.5
        ...
        elseif(IHA.eq.4) then
          ...
          SEPS=S**0.2D0 
          BEL=4.5
          SIGTMP(I,0)=XPAR(IPR)*SEPS*0.550D0
        ELSE
          SIGTMP(I,0)=XPAR(IPR)*SEPS+YPAR(IPR)*SETA
          BEL=2D0*BHA+2D0*BHB+4D0*SEPS-4.2D0
          write(*,*), IHA, IPR, SIGTMP(I,0), I
        ENDIF
...
C   In principle the power is rho:2.575, phi:2.4 and J/psi:2.55
      ELSEIF(MINT(101).EQ.4.AND.MINT(102).EQ.1) THEN
        DO 140 I=1,4
         IF(MINT(107).EQ.2) THEN
            VINT(317)=(PMHAD(I)**2/(PMHAD(I)**2+VINT(307)))**expo(I)
         ELSE
     ...
     &     (1D0/PARP(15)**2-1D0/VINT(154)**2)*VINT(317)
         ...
         ENDIF
         I1=MAX(1,I-1)
C... M.D.Baker 2018-03-14 Allow selection of single Vector Meson in BeAGLE
         DO 130 J=0,5
            IF (IDXVECBP.EQ.0 .OR. IDXVECBP.EQ.I) THEN 
      ...
      ELSEIF(MINT(101).EQ.1.AND.MINT(102).EQ.4) THEN
        DO 170 I=1,4
         IF(MINT(108).EQ.2) THEN
            VINT(317)=(PMHAD(I)**2/(PMHAD(I)**2+VINT(308)))**expo(I)
         ELSE
          VINT(317)=16D0*PARP(15)**2*VINT(154)**2/
