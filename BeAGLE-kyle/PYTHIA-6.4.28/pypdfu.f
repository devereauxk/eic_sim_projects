...
C...Information about LT from lab to nucleon rest frame
      COMMON /LABTONR/ pgamma, pbeta, pbeamP, pbeamN, idNuc, lName
      DOUBLE PRECISION pgamma, pbeta
      REAL pbeamP, pbeamN
      INTEGER idNuc
      CHARACTER*10 lName

c...Global label to tell beam side 1 for projectile, 2 for target
c...added by liang 5/27/12      
      COMMON /SIDEINF/ NSIDE     
...
C...Modified by liang to add nuclear correction 1/6/12*************
          IF((CHANUM.GT.1D0).AND.(ORDER/100.GT.0).AND.(NSIDE.EQ.2)) THEN
...
C     MDB - Use nearest A value as advised by H. Paukkunen 10/3/17
              CALL STRUCTA(XX,QQ,ANEAR(INUMOD),UPV,DNV,USEA,DSEA,STR,
     &             CHM,BOT,TOP,GLU)
...
C             print*,'Now run NLO nuclear pdf'
     ...
     &             TOP,GLU)
          ...
          ELSE
            CALL STRUCTM(XX,QQ,UPV,DNV,USEA,DSEA,STR,CHM,BOT,TOP,GLU)
          ENDIF
C...2016-11-16 MDB Change isospin/flavor handling. 
C...               Now we account for which flavor nucleon is struck
C...               rather than averaging.
C
          A=INUMOD
          Z=CHANUM
CC...OLD Isospin transformation for nucleus A with Z protons************     
Cc...OLD make isospin average used for both Z>1 and Z=1 (.eg.Deutrum)
C         IF( A.GT.1 ) THEN
C            UPVp=UPV
C            UPVn=DNV
C            USEAp=USEA
...
C            DSEAp=DSEA
C            DSEAn=USEA
C            UPV=Z/A*UPVp+(A-Z)/A*UPVn
C            USEA=Z/A*USEAp+(A-Z)/A*USEAn
C            DNV=Z/A*DNVp+(A-Z)/A*DNVn
C            DSEA=Z/A*DSEAp+(A-Z)/A*DSEAn
C         ENDIF
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
          IF (idNuc.EQ.2112) THEN
            UPVn=DNV
            USEAn=DSEA
            DNVn=UPV
            DSEAn=USEA
